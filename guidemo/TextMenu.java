...
public class TextMenu extends JMenu {
	...
	private JRadioButtonMenuItem left; // controls if text is left justified or not.
	...
	public TextMenu(DrawPanel owner) {
		...
		final JMenuItem lineHeightMultiplier = new JMenuItem("Set line height...");
		lineHeightMultiplier.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent evt)
			{
				double mult = panel.getTextItem().getLineHeightMultiplier();
				String userIn = JOptionPane.showInputDialog(panel,"Enter line height multiplier? ", mult);
				if (userIn != null && userIn.trim().length() > 0)
				{
					try
					{
						double newLineSpace = Double.parseDouble(userIn.trim()); // can throw NumberFormatException
						panel.getTextItem().setLineHeightMultiplier(newLineSpace); // can throw IllegalArgumentException
						panel.repaint();
						}
					catch (Exception e)
					{
						JOptionPane.showMessageDialog(panel, userIn + " is not a legal value. " + "Please enter a positive integer.");
						}
					}
				}
			});

...
public class GuiDemo extends JFrame{
	...
	public GuiDemo() {
		...
		drawPanel.getTextItem().setText(
				"Thank you for being a supportive instructor, I had lots of fun! Thank you!"
			);
		...
		//add stamper menu option 
		IconSupport stamper = new IconSupport(drawPanel);
		menuBar.add(stamper.createMenu());
	...
	public JToolBar makeToolbar(boolean horizontal) {
		JToolBar mbar = new JToolBar( horizontal? JToolBar.HORIZONTAL : JToolBar.VERTICAL);
		mbar.add(new ChooseBackgroundAction("Mandelbrot"));
		mbar.add(new ChooseBackgroundAction("Earthrise"));
		mbar.add(new ChooseBackgroundAction("Sunset"));
		mbar.add(new ChooseBackgroundAction("Cloud"));
		mbar.add(new ChooseBackgroundAction("Eagle_nebula"));
		mbar.addSeparator();
		mbar.add(new ChooseBackgroundAction("Custom..."));
		mbar.addSeparator();
		mbar.add(new ChooseBackgroundAction("Color..."));
		mbar.addSeparator();
		mbar.add(gradientOverlayCheckbox);
		mbar.addSeparator(new Dimension(15,0));
		return mbar;
	}
    ...
    private class ChooseBackgroundAction extends AbstractAction {
    	...
    	ChooseBackgroundAction(String text) {
    		...
    		if (text.equals("Color...")) {
    			//create image to use for color action
    			putValue(Action.SHORT_DESCRIPTION, "<html>Use a solid color for background<br>instead of an image.</html>");
    			BufferedImage col = new BufferedImage(32,32,BufferedImage.TYPE_INT_ARGB);
